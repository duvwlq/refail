...
기존 운영 지표 API는 관리자 상태 확인 1회와 게시글·후속 기록·신고 집계 6회를 순서대로 실행했다. 각 집계를 스칼라 서브쿼리로 묶어 한 번에 조회하도록 변경했다.

- API 전체 SQL: `7개 → 2개`
- 관리자 현재 상태 확인 SQL은 권한 회수를 위해 유지한다.
- 지표 집계 SQL은 `6개 → 1개`로 통합한다.
- `AdminMetricsQueryIntegrationTest`가 응답 값과 최대 SQL 2개를 함께 검증한다.

게시글 1만 건, 후속 기록 1천 건, 신고 200건을 생성한 MySQL 8.4 환경에서 3회 워밍업 후 10회 측정한 결과다.

| 방식 | SQL 수 | 평균 | p95 |
| --- | ---: | ---: | ---: |
| 개별 집계 | 6 | 6.75ms | 7.03ms |
| 통합 집계 | 1 | 3.18ms | 4.08ms |

현재 관리자만 호출하고 통합 쿼리의 평균이 3.18ms이므로 TTL 캐시는 추가하지 않았다. 캐시를 도입하면 지표 지연 허용시간과 모든 쓰기 경로의 무효화 정책이 추가되는데, 현재 얻는 이점보다 복잡도가 크다.
