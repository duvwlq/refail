...
- 운영 브라우저는 Caddy HTTPS 동일 출처의 `/api` 경로만 사용하므로 CORS와 쿠키 출처가 분리되지 않는다.
...
- `scripts/test-production-deployment.ps1`: 실제 HTTPS 프록시에서 Secure·HttpOnly·SameSite·Path, 회전과 로그아웃 폐기 검증
...
운영 Compose는 Spring Boot 포트를 호스트에 공개하지 않고 Caddy만 접근하게 한다. 이 경계가 유지될 때만 전달 IP 헤더를 요청 제한에 사용한다.
