...
| 자동 검증 | 백엔드 37개 테스트, 프론트엔드 lint·production build |
...
백엔드 전체 테스트에는 Docker 기반 MySQL 8.4 Testcontainers 테스트가 포함됩니다. Flyway 마이그레이션, 동시 공감·신고·토큰 갱신 정합성, 사용자 운영 이력, 삭제 사용자와 비활성 카테고리 정책을 실제 MySQL에서 검증합니다. 최종 검증 기준 37개 테스트가 모두 통과했습니다.
